...
/* ═══ LE BOUTON FLOTTANT KO-FI ════════════════════════════════════════
   Le second widget que Ko-fi propose, `overlay-widget.js`, celui-là est
   intégrable : il ne rend pas par `document.write` mais pose un bouton
   flottant et ouvre son panneau dans une iframe. C'est celui de Niko,
   avec ses réglages — texte, fond `#00b9fe`, encre blanche.

   Ce qu'il coûte, mesuré au navigateur : **10 requêtes vers trois
   domaines tiers pour 5 Ko** — le script, deux feuilles de style, le logo
   Ko-fi, et **DM Sans chez Google Fonts**. Ce dernier point mérite d'être
   dit : le site auto-héberge ses deux polices, et ce bouton en fait venir
   une troisième de chez Google, avec l'adresse IP du visiteur qui va
   avec. C'est le seul appel de ce genre sur le site.

   **Le calage est à nous, et il n'est pas facultatif.** Le widget se pose
   en bas à gauche, à `bottom:0` : il recouvrait la barre de progression,
   et sur téléphone la barre d'installation par-dessus le marché. Il
   reprend donc les deux mesures que le bouton « remonter en haut »
   utilise déjà — `--hud-h`, la hauteur de la barre de progression, et
   `--cg-bar-h` sous la classe `cg-bar`, celle de la barre du bas quand
   il y en a une. Trois éléments partagent ce coin ; ils se rangent tous
   sur les mêmes variables.

   Le bouton du pied de page reste : celui-ci se ferme, se bloque par un
   filtre de publicité, et ne paraît pas si Ko-fi ne répond pas.
   ══════════════════════════════════════════════════════════════════ */
(function(){
  'use strict';
  var FR = document.documentElement.lang !== 'en';
  ...
  var CALE = [
    /* `!important` parce que la règle qu'on écrase est celle du widget,
       posée dans sa propre feuille et qu'on ne peut pas modifier. */
    '.floatingchat-container-wrap,.floatingchat-container-wrap-mobi{',
    '  bottom:calc(var(--hud-h,0px) + 10px)!important;left:12px!important}',
    'html.cg-bar .floatingchat-container-wrap,',
    'html.cg-bar .floatingchat-container-wrap-mobi{',
    '  bottom:calc(var(--hud-h,0px) + var(--cg-bar-h,64px) + 10px)!important}'
  ].join('');
  ...
  function pose(){
    ...
    s.onload = function(){
      try {
        window.kofiWidgetOverlay.draw('chronologeek', {
          'type': 'floating-chat',
          'floating-chat.donateButton.text': FR ? 'Soutenez-moi' : 'Support me',
          'floating-chat.donateButton.background-color': '#00b9fe',
          'floating-chat.donateButton.text-color': '#fff'
        });
      } catch (e) {}
    };
    /* Pas de `onerror` à traiter : si Ko-fi ne répond pas, il ne se passe
       rien, et le bouton du pied de page est toujours là. */
    document.head.appendChild(s);
  }

  /* Après `load`, et pas avant : ce bouton n'est pas le contenu de la page,
     il n'a rien à faire sur le chemin d'arrivée. Le délai le laisse aussi
     passer après la barre d'installation, qui se construit à 1,2 s et dont
     il doit connaître la hauteur. */
  if (document.readyState === 'complete') setTimeout(pose, 1600);
  else addEventListener('load', function(){ setTimeout(pose, 1600); });
})();
