/* ═══ LE FRANÇAIS DE JURASSIC WORLD ═══════════════════════════
   Produit `e-jurassic.html` et `data-jurassic.js` à partir des fichiers
   anglais. C'est le sixième script du dossier qui traduit dans ce sens,
   après Star Trek, The Walking Dead, Dragon Age, Assassin's Creed et DC
   Animation, et pour la même raison : Niko a écrit ce guide-là en anglais.

   NE JAMAIS INSCRIRE JURASSIC WORLD DANS `traduire-pages.mjs`. Ce
   script-là produit l'anglais depuis le français : il écraserait
   `en-jurassic.html`, c'est-à-dire la source, avec une retraduction de sa
   propre sortie — sans erreur et sans message.

   ON NE TRADUIT QUE CE QUI EST PROPRE À JURASSIC WORLD. Tout ce qui
   appartient au gabarit du site — navigation, pied de page, filtres,
   niveaux, progression, les deux parcours, boutons — existe déjà en
   français dans les autres protos. Le lexique vient de trois sources :

   - les paires de protos `e-*.html` / `en-*.html`, parallèles ligne à
     ligne. **Star Wars en fait partie, et en tête** : cette page-ci est
     bâtie sur `en-starwars.html`, à une trentaine de substitutions près,
     et c'est la paire qui rend le plus — la question des deux parcours,
     la bascule, le tamis, le HUD. Avatar Legends suit, pour le badge VO ;
   - les paires de données `data-*.js` / `data-*-en.js`, appariées par
     chemin de clé — c'est ce qui rend les libellés de `CG.t`. Star Trek
     et Avatar Legends y sont pour leurs badges de type : `short` vient
     de l'un, `roman` de l'autre ;
   - `JW_RETROUVES`, les titres d'exploitation française pris chez TMDB
     en fr-FR, et celui du roman de Claire au catalogue de la BnF.

   Ce qui reste — l'accroche, les deux repères de lecture, ce qui est
   écarté, les quatre ères, les badges et les trois résumés de romans — est
   écrit, et vit dans `traductions-jurassic.mjs`.

       node _proto/traduire-jurassic.mjs          écrit les deux fichiers
       node _proto/traduire-jurassic.mjs --check  n'écrit rien
       node _proto/traduire-jurassic.mjs --detail liste ce qui manque
...
import { JW_IDENTIQUES, JW_RETROUVES, JW_TRADUCTIONS, JW_GABARITS }
  from './traductions-jurassic.mjs';
...
const identiques = new Set(JW_IDENTIQUES.map(net));
...
/* Star Wars d'abord : c'est la page dont celle-ci est bâtie, et la
   première vue gagne en cas de collision. Les deux parcours, la question
   posée à l'arrivée et la bascule n'ont de français que là — et chez
   Avatar Legends, qui suit pour le badge VO. */
const PAIRES_PAGES = [
...
/* ── 2 bis. les accroches, nœud de texte par nœud de texte ─────────
   ...
   forme que ceux-ci. */
const PAIRES_ACCROCHES = [...PAIRES_DONNEES, ['data-dcanimation.js', 'data-dcanimation-en.js']];
let accrochesLues = 0;
...
for (const [en, fr] of JW_RETROUVES) ajoute(en, fr);

/* ── 4. ce qui est écrit ─────────────────────────────────── */
const ECRIT = new Map(JW_TRADUCTIONS.map(([en, fr]) => [net(en), fr]));
...
for (const [en, fr] of JW_TRADUCTIONS) {
  const a = (en.match(/\n/g) || []).length, b = (String(fr).match(/\n/g) || []).length;
  if (a !== b) {
    throw new Error(`traductions-jurassic : ${a} saut(s) de ligne en anglais, ${b} en français`
      + `\n  ${en.slice(0, 90)}…`);
  }
}
...
const ecrites = [];      // venu de traductions-jurassic.mjs, à relire
...
function gabarit(v) {
  for (const [re, rendu] of JW_GABARITS) {
    const m = v.match(re);
...
const SRC = lire('_proto/data-jurassic-en.js');
const boite = evalue(SRC);
const CG_EN = boite.CG, JW_EN = boite.JURASSIC;
...
const JW_FR = traduitObjet(JW_EN, 'JURASSIC');
...
memesCles(JW_EN, JW_FR, 'JURASSIC');
...
/* 2. le champ resté anglais : si toutes les valeurs d'un champ sont
   ...
   `title` y échappe : Jurassic Park, Jurassic World et les deux romans
   de Maisie Lockwood, sans édition française, gardent leur titre. */
...
{
  const ce = parChamp(JW_EN), cf = parChamp(JW_FR);
...
function remplaceBloc(src, decl, valeur) {
  ...
  if (i < 0) throw new Error(`bloc introuvable dans data-jurassic-en.js : ${decl}`);
...
const ENTETE_FR = `/* Version francaise de data-jurassic-en.js — produite par
   traduire-jurassic.mjs. Jurassic World est le sixieme univers dont la source
   ...
   protos, jamais retraduit ; les titres d'oeuvres viennent de TMDB en fr-FR, et
   celui du roman de Claire du catalogue de la BnF. Ce qui est propre a cet
   univers est ecrit dans traductions-jurassic.mjs. Ne pas editer a la main :
   relancer le script. */`;

let sortie = SRC;
sortie = remplaceBloc(sortie, 'window.CG=', CG_FR);
sortie = remplaceBloc(sortie, 'window.JURASSIC=', JW_FR);
...
const PAGE = lire('_proto/en-jurassic.html');
...
let pageFR = pageTraduite
  ...
  .replace(/(<script\s+src=")data-jurassic-en\.js(")/, '$1data-jurassic.js$2')
   ...
   tombent sous le pixel. Même nombre de lignes que le bloc anglais. */
{
  ...
  if (!re.test(pageFR)) throw new Error('pas de sélecteur de langue dans en-jurassic.html');
  pageFR = pageFR.replace(re, (bloc) => {
    const lignes = bloc.split('\n');
    const out = [
      '<a class="icon-btn lang-btn" href="en-jurassic.html" aria-label="Switch to English" title="Switch to English">',
...
console.log(`  retrouvés : ${JW_RETROUVES.length} titres, `
  + `gabarits ${gabaritsVus.size}/${JW_GABARITS.length}`);
...
fs.writeFileSync(path.join(RACINE, '_proto/a-traduire-jurassic.json'),
...
if (!CHECK) {
  ...
  fs.writeFileSync(path.join(RACINE, '_proto/data-jurassic.js'), sortie, 'utf8');
  fs.writeFileSync(path.join(RACINE, '_proto/e-jurassic.html'), pageFR, 'utf8');
  console.log('\n  → _proto/data-jurassic.js\n  → _proto/e-jurassic.html');
  fs.writeFileSync(path.join(RACINE, '_proto/a-relire-jurassic.json'),
