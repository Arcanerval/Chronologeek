/* ═══ LE FRANÇAIS DE RESIDENT EVIL ═══════════════════════════
   Produit `e-residentevil.html` et `data-re.js` à partir des fichiers
   anglais. C'est le huitième script du dossier qui traduit dans ce sens,
   après Star Trek, The Walking Dead, Dragon Age, Assassin's Creed, DC
   Animation, Jurassic World et The Witcher, et pour la même raison : Niko
   a écrit ce guide-là en anglais.

   NE JAMAIS INSCRIRE RESIDENT EVIL DANS `traduire-pages.mjs`. Ce
   script-là produit l'anglais depuis le français : il écraserait
   `en-residentevil.html`, c'est-à-dire la source, avec une retraduction de sa
   propre sortie — sans erreur et sans message.

   ON NE TRADUIT QUE CE QUI EST PROPRE À RESIDENT EVIL. Tout ce qui
   appartient au gabarit du site — navigation, pied de page, filtres,
   niveaux, progression, boutons — existe déjà en français dans les autres
   protos. Le lexique vient de trois sources :

   - les paires de protos `e-*.html` / `en-*.html`, parallèles ligne à
     ligne. **Assassin's Creed en fait partie, et en tête** : cette
     page-ci en est bâtie, à une trentaine de substitutions près, et c'est
     la paire qui rend le plus — le tamis, le HUD, la fiche RAWG, les
     badges de type du jeu et du DLC ;
   - les paires de données `data-*.js` / `data-*-en.js`, appariées par
     chemin de clé — c'est ce qui rend les libellés de `CG.t`. Dragon Age
     y est pour ses badges de type : `filmanim`, `anime` et `video`
     viennent de lui ;
   - `RE_RETROUVES`, les titres d'exploitation française, pris chez TMDB
     en fr-FR pour les films d'animation et la série.

   Ce qui reste — l'accroche, les trois repères de lecture, ce qui est
   écarté, les quatre ères et les badges — est écrit, et vit dans
   `traductions-residentevil.mjs`.

       node _proto/traduire-residentevil.mjs          écrit les deux fichiers
       node _proto/traduire-residentevil.mjs --check  n'écrit rien
       node _proto/traduire-residentevil.mjs --detail liste ce qui manque
...
import { RE_IDENTIQUES, RE_RETROUVES, RE_TRADUCTIONS, RE_GABARITS }
  from './traductions-residentevil.mjs';
...
const identiques = new Set(RE_IDENTIQUES.map(net));
...
/* Assassin's Creed d'abord : c'est la page dont celle-ci est bâtie, et
   la première vue gagne en cas de collision. Le tamis, le HUD et la fiche
   RAWG n'ont de français que là — et chez Dragon Age, qui suit pour ses
   badges de type. */
const PAIRES_PAGES = [
  ['e-assassinscreed.html', 'en-assassinscreed.html'],
  ['e-dragonage.html', 'en-dragonage.html'],
  ['e-dcanimation.html', 'en-dcanimation.html'],
  ...
  ['e-witcher.html', 'en-witcher.html'],
...
/* ── 2 bis. les accroches, nœud de texte par nœud de texte ─────────
   L'accroche d'une page vit dans les données, en un seul bloc de HTML :
   la paire de données ci-dessus l'apprend donc entière, et une accroche
   neuve n'y retrouve rien — alors que ses intitulés sont ceux des autres
   univers, mot pour mot : « How to read this », « FLASHBACKS », les deux
   phrases des parcours, « What's left out, and why ». On apparie donc les
   nœuds de texte par rang, et seulement si les deux versions en ont
   autant — une accroche remaniée d'un côté ne rend rien plutôt que faux.
   DC Animation y entre pour ses repères, écrits par Niko dans la même
   forme que ceux-ci. */
const PAIRES_ACCROCHES = [...PAIRES_DONNEES, ['data-twd.js', 'data-twd-en.js']];
let accrochesLues = 0;
for (const [fr, en] of PAIRES_ACCROCHES) {
  const bf = evalue(lire(`_proto/${fr}`)), be = evalue(lire(`_proto/${en}`));
  const univ = b => Object.values(b).find(v => v && typeof v === 'object' && typeof v.notes === 'string');
  const uf = univ(bf), ue = univ(be);
  if (!uf || !ue) continue;
  const noeuds = h => [...h.matchAll(TEXTES)].map(m => m[1]).filter(v => v.trim() && /\p{L}/u.test(v));
  const nf = noeuds(uf.notes), ne = noeuds(ue.notes);
  if (nf.length !== ne.length) continue;
  ne.forEach((v, i) => ajoute(v, nf[i]));
  accrochesLues++;
}

/* ── 3. les titres, retrouvés chez TMDB et à la BnF ──────────────
   ...
   une API ou un catalogue plutôt que dans une page. Ils sont ajoutés en
   dernier, donc ils ne prennent la place de rien. */
for (const [en, fr] of RE_RETROUVES) ajoute(en, fr);

/* ── 4. ce qui est écrit ─────────────────────────────────── */
const ECRIT = new Map(RE_TRADUCTIONS.map(([en, fr]) => [net(en), fr]));
...
for (const [en, fr] of RE_TRADUCTIONS) {
  const a = (en.match(/\n/g) || []).length, b = (String(fr).match(/\n/g) || []).length;
  if (a !== b) {
    throw new Error(`traductions-residentevil : ${a} saut(s) de ligne en anglais, ${b} en français`
      + `\n  ${en.slice(0, 90)}…`);
  }
}
...
const ecrites = [];      // venu de traductions-residentevil.mjs, à relire
const gabaritsVus = new Set();

/* Les gabarits d'abord : « Season 1 Episodes 1-3 » n'a pas à figurer
   dans une table, c'est une forme. Les sous-items en sortent, et le
   décompte « 12 / 12 shown » avec eux. */
function gabarit(v) {
  for (const [re, rendu] of RE_GABARITS) {
    const m = v.match(re);
...
const SRC = lire('_proto/data-re-en.js');
const boite = evalue(SRC);
const CG_EN = boite.CG, REV_EN = boite.RE;
...
const TECHNIQUES = new Set([
  ...
  'tmdbKey', 'rawgKey', 'icon', 'c', 'isbn',
  /* les deux parcours : un identifiant, une liste d'identifiants, des noms
     de champ — traduits, ils ne désigneraient plus rien */
  'ref', 'covers', 'drop', 'universe_key',
]);

/* Une date de timeline ne se traduit jamais — « 1993 », « 2015-2016 ».
   Le champ est déclaré ici plutôt que dans la liste des identiques. */
...
/* `CG` se traduit clé par clé, jamais remplacé par celui d'un autre
   ...
   les quatre types de médias, le repère FLASHBACK. */
const alertes = [];
const CG_FR = traduitObjet(CG_EN, 'CG');
CG_FR.t.lang = 'fr';
CG_FR.t.locale = 'fr-FR';
const REV_FR = traduitObjet(REV_EN, 'RE');
...
/* 2. le champ resté anglais : si toutes les valeurs d'un champ sont
   ...
   `title` y échappe : aucune œuvre de ce guide n'a de titre français —
   les jeux, les films d'animation et les deux mangas gardent le leur. Et
   `phase` avec lui : « PHASE 1 » s'écrit pareil dans les deux langues,
   comme chez Marvel. */
...
{
  const ce = parChamp(REV_EN), cf = parChamp(REV_FR);
...
/* 3. la variété : quatre titres d'ère distincts d'un côté et un seul de
...
function remplaceBloc(src, decl, valeur) {
  ...
  if (i < 0) throw new Error(`bloc introuvable dans data-re-en.js : ${decl}`);
...
const ENTETE_FR = `/* Version francaise de data-re-en.js — produite par
   traduire-residentevil.mjs. Resident Evil est le huitieme univers dont la
   source est anglaise : Niko a ecrit ce guide-la dans cette langue, et le
   francais en descend. Tout ce qui appartient au gabarit du site est retrouve
   dans les autres protos, jamais retraduit. Aucune oeuvre du guide n'a de titre
   francais : les jeux, les films d'animation et les deux mangas gardent le leur.
   Ce qui est propre a cet univers est ecrit dans traductions-residentevil.mjs.
   Ne pas editer a la main : relancer le script. */`;

let sortie = SRC;
sortie = remplaceBloc(sortie, 'window.CG=', CG_FR);
sortie = remplaceBloc(sortie, 'window.RE=', REV_FR);
...
const PAGE = lire('_proto/en-residentevil.html');
...
let pageFR = pageTraduite
  ...
  .replace(/(<script\s+src=")data-re-en\.js(")/, '$1data-re.js$2')
   ...
   tombent sous le pixel. Même nombre de lignes que le bloc anglais. */
{
  ...
  if (!re.test(pageFR)) throw new Error('pas de sélecteur de langue dans en-residentevil.html');
  pageFR = pageFR.replace(re, (bloc) => {
    const lignes = bloc.split('\n');
    const out = [
      '<a class="icon-btn lang-btn" href="en-residentevil.html" aria-label="Switch to English" title="Switch to English">',
      '        <svg viewBox="0 0 60 40" aria-hidden="true"><clipPath id="enus"><path d="M0 0h60L0 40z"/></clipPath><clipPath id="enuk"><path d="M60 0v40H0z"/></clipPath><g clip-path="url(#enus)"><path d="M0 0h60v40H0z" fill="#B22234"/><path d="M0 3.08h60v3.08H0zM0 9.23h60v3.08H0zM0 15.38h60v3.08H0zM0 21.54h60v3.08H0zM0 27.69h60v3.08H0zM0 33.85h60v3.08H0z" fill="#fff"/><path d="M0 0h24v21.54H0z" fill="#3C3B6E"/><g fill="#fff"><circle cx="3" cy="3.6" r="1.7"/><circle cx="9" cy="3.6" r="1.7"/><circle cx="15" cy="3.6" r="1.7"/><circle cx="21" cy="3.6" r="1.7"/><circle cx="3" cy="10.8" r="1.7"/><circle cx="9" cy="10.8" r="1.7"/><circle cx="15" cy="10.8" r="1.7"/><circle cx="21" cy="10.8" r="1.7"/><circle cx="3" cy="17.9" r="1.7"/><circle cx="9" cy="17.9" r="1.7"/><circle cx="15" cy="17.9" r="1.7"/><circle cx="21" cy="17.9" r="1.7"/></g></g><g clip-path="url(#enuk)" fill="none"><path d="M0 0h60v40H0z" fill="#012169"/><path d="M0 0l60 40m0-40L0 40" stroke="#fff" stroke-width="9"/><path d="M0 0l60 40m0-40L0 40" stroke="#C8102E" stroke-width="3.6"/><path d="M30 0v40" stroke="#fff" stroke-width="13"/><path d="M0 20h60" stroke="#fff" stroke-width="17"/><path d="M30 0v40" stroke="#C8102E" stroke-width="7"/><path d="M0 20h60" stroke="#C8102E" stroke-width="9"/></g><path d="M60 0L0 40" stroke="#fff" stroke-width="1.8" fill="none"/></svg>',
    ];
...
console.log(`  lexique : ${table.size} textes retrouvés (${pagesLues} paires de pages, `
  + `${cgParChemin.size} clés de données, ${accrochesLues} accroches), ${identiques.size} identiques`
  + (collisions.size ? `, ${collisions.size} ambigus` : ''));
console.log(`  retrouvés : ${RE_RETROUVES.length} titres, `
  + `gabarits ${gabaritsVus.size}/${RE_GABARITS.length}`);
...
fs.writeFileSync(path.join(RACINE, '_proto/a-traduire-residentevil.json'),
...
if (!CHECK) {
  ...
  fs.writeFileSync(path.join(RACINE, '_proto/data-re.js'), sortie, 'utf8');
  fs.writeFileSync(path.join(RACINE, '_proto/e-residentevil.html'), pageFR, 'utf8');
  console.log('\n  → _proto/data-re.js\n  → _proto/e-residentevil.html');
  fs.writeFileSync(path.join(RACINE, '_proto/a-relire-residentevil.json'),
