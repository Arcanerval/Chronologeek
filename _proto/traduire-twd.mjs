/* ═══ LE FRANÇAIS DE THE WALKING DEAD ════════════════════════════════
   Produit `e-twd.html` et `data-twd.js` à partir des fichiers anglais.
   C'est le second script du dossier qui traduit dans ce sens, après
   `traduire-startrek.mjs`, et pour la même raison : Niko a écrit ce
   guide-là en anglais. Les quatre premiers univers s'écrivent en
   français et l'anglais s'en déduit ; ici la source est anglaise.

   NE JAMAIS INSCRIRE THE WALKING DEAD DANS `traduire-pages.mjs`. Ce
   script-là produit l'anglais depuis le français : il écraserait
   `en-twd.html`, c'est-à-dire la source, avec une retraduction de sa
   propre sortie — sans erreur et sans message.

   ON NE TRADUIT QUE CE QUI EST PROPRE À THE WALKING DEAD. Tout ce qui
   appartient au gabarit du site — navigation, pied de page, filtres,
   niveaux, progression, boutons — existe déjà en français dans les dix
   ...
   - les paires de protos `e-*.html` / `en-*.html`, parallèles ligne à
     ligne puisque les unes sortent des autres. **Star Trek en fait
     partie** : cette page-ci est bâtie sur la sienne, et c'est de loin
     la paire qui rend le plus — bandeaux, tamis, HUD, badges ;
   - les paires de données `data-*.js` / `data-*-en.js`, appariées par
     chemin de clé — c'est ce qui rend les libellés de `CG.t`.

   Ce qui reste — l'accroche, les trois repères de lecture, les quatorze
   phases, les badges — est écrit, et vit dans `traductions-twd.mjs`.

       node _proto/traduire-twd.mjs          écrit les deux fichiers
       node _proto/traduire-twd.mjs --check  n'écrit rien
       node _proto/traduire-twd.mjs --detail liste ce qui manque
...
const identiques = new Set(TWD_IDENTIQUES.map(net));
...
/* Star Trek d'abord : c'est la page dont celle-ci est bâtie, et la
   première vue gagne en cas de collision. « Reveal », « Minor spoilers »
   ou « left to watch » y ont exactement le sens qu'ils ont ici. */
const PAIRES_PAGES = [
...
/* ── 3. ce qui est écrit ──────────────────────────────────────────── */
const ECRIT = new Map(TWD_TRADUCTIONS.map(([en, fr]) => [net(en), fr]));
...
for (const [en, fr] of TWD_TRADUCTIONS) {
  const a = (en.match(/\n/g) || []).length, b = (String(fr).match(/\n/g) || []).length;
  if (a !== b) {
    throw new Error(`traductions-twd : ${a} saut(s) de ligne en anglais, ${b} en français`
      + `\n  ${en.slice(0, 90)}…`);
  }
}
...
const ecrites = [];      // venu de traductions-twd.mjs, à relire
...
function gabarit(v) {
  for (const [re, rendu] of TWD_GABARITS) {
    const m = v.match(re);
...
const SRC = lire('_proto/data-twd-en.js');
const boite = evalue(SRC);
const CG_EN = boite.CG, TWD_EN = boite.TWD;
...
const TWD_FR = traduitObjet(TWD_EN, 'TWD');
...
/* 2. le champ resté anglais : si toutes les valeurs d'un champ sont
   ...
   Deux champs y échappent, et c'est propre à cet univers : les quinze
   œuvres de The Walking Dead n'ont pas un seul titre français, et
   « PHASE 12 » s'écrit pareil des deux côtés. Le contrôle dirait vrai —
   ces champs n'ont pas bougé — sans rien signaler d'anormal.

   `title` porte aussi les quatorze titres de phase, qui eux se
   traduisent : le garde-fou de variété, plus bas, les surveille. */
...
{
  const ce = parChamp(TWD_EN), cf = parChamp(TWD_FR);
...
function remplaceBloc(src, decl, valeur) {
  ...
  if (i < 0) throw new Error(`bloc introuvable dans data-twd-en.js : ${decl}`);
...
const ENTETE_FR = `/* Version francaise de data-twd-en.js — produite par traduire-twd.mjs.
   The Walking Dead est le second univers dont la source est anglaise, apres
   Star Trek : Niko a ecrit ce guide-la dans cette langue, et le francais en
   descend. Tout ce qui appartient au gabarit du site est retrouve dans les dix
   autres protos, jamais retraduit ; ce qui est propre a cet univers —
   l'accroche, les reperes de lecture, les quatorze phases, les badges — est
   ecrit dans traductions-twd.mjs. Ne pas editer a la main : relancer le
   script. Les quinze oeuvres gardent leur titre : aucune n'a de titre
   francais. */`;

let sortie = SRC;
sortie = remplaceBloc(sortie, 'window.CG=', CG_FR);
sortie = remplaceBloc(sortie, 'var DATA_TWD=', TWD_FR);
...
const PAGE = lire('_proto/en-twd.html');
...
let pageFR = pageTraduite
  ...
  .replace(/(<script\s+src=")data-twd-en\.js(")/, '$1data-twd.js$2')
   ...
   tombent sous le pixel. Même nombre de lignes que le bloc anglais. */
{
  ...
  if (!re.test(pageFR)) throw new Error('pas de sélecteur de langue dans en-twd.html');
  pageFR = pageFR.replace(re, (bloc) => {
    const lignes = bloc.split('\n');
    const out = [
      '<a class="icon-btn lang-btn" href="en-twd.html" aria-label="Switch to English" title="Switch to English">',
...
console.log(`  gabarits : ${gabaritsVus.size}/${TWD_GABARITS.length} employés`);
...
fs.writeFileSync(path.join(RACINE, '_proto/a-traduire-twd.json'),
