/* ═══ LE FRANÇAIS DE DC ANIMATION ════════════════════════════
   Produit `e-witcher.html` et `data-witcher.js` à partir des
   ...
   `en-witcher.html`, c'est-à-dire la source, avec une retraduction de
   ...
   - `W_RETROUVES`, les titres et les 180 sous-items d'épisodes, pris
   ...
   dans `traductions-witcher.mjs`.

       node _proto/traduire-witcher.mjs          écrit les deux fichiers
       node _proto/traduire-witcher.mjs --check  n'écrit rien
       node _proto/traduire-witcher.mjs --detail liste ce qui manque
...
const identiques = new Set(W_IDENTIQUES.map(net));
...
const PAIRES_PAGES = [
  ...
  ['e-jurassic.html', 'en-jurassic.html'],
...
const PAIRES_DONNEES = [
  ...
  ['data-jurassic.js', 'data-jurassic-en.js'],
...
for (const [en, fr] of W_RETROUVES) ajoute(en, fr);

/* ── 4. ce qui est écrit ─────────────────────────────────── */
const ECRIT = new Map(W_TRADUCTIONS.map(([en, fr]) => [net(en), fr]));
...
for (const [en, fr] of W_TRADUCTIONS) {
  const a = (en.match(/\n/g) || []).length, b = (String(fr).match(/\n/g) || []).length;
  if (a !== b) {
    throw new Error(`traductions-witcher : ${a} saut(s) de ligne en anglais, ${b} en français`
      + `\n  ${en.slice(0, 90)}…`);
  }
}
...
const ecrites = [];      // venu de traductions-witcher.mjs, à relire
...
function gabarit(v) {
  for (const [re, rendu] of W_GABARITS) {
    const m = v.match(re);
...
const SRC = lire('_proto/data-witcher-en.js');
const boite = evalue(SRC);
const CG_EN = boite.CG, W_EN = boite.WITCHER;
...
const W_FR = traduitObjet(W_EN, 'WITCHER');
...
const ZDESC_EN = W_EN.zoneDesc || {};
{
  const zones = new Map();
  W_EN.eras.forEach((e, i) => { if (e.zone) zones.set(e.zone, W_FR.eras[i].zone); });
  const avant = W_FR.zoneDesc || {};
  const apres = {};
  for (const [k, v] of Object.entries(avant)) apres[zones.get(k) || k] = v;
  W_FR.zoneDesc = apres;
  /* Le controle ne vaut que pour une zone QUI EN AVAIT UNE : la page
     Witcher n'a qu'une zone et Niko ne lui a pas ecrit de description —
     exiger la traduction d'un texte qui n'existe pas ferait sortir le
     script en erreur a chaque passage. */
  for (const e of W_FR.eras) {
    const avant = e.zone && Object.keys(ZDESC_EN).length
      ? W_EN.eras.find((x, i) => W_FR.eras[i] === e)?.zone : null;
    if (avant && ZDESC_EN[avant] !== undefined && W_FR.zoneDesc[e.zone] === undefined) {
...
/* `zoneDesc` est le seul endroit du fichier dont les CLÉS changent : ce
   ...
   tient la place. */
const CLES_LIBRES = new Set(['WITCHER.zoneDesc']);
...
{
  const ce = parChamp(W_EN), cf = parChamp(W_FR);
...
function remplaceBloc(src, decl, valeur) {
  ...
  if (i < 0) throw new Error(`bloc introuvable dans data-witcher-en.js : ${decl}`);
...
const ENTETE_FR = `/* Version francaise de data-witcher-en.js — produite par
   traduire-witcher.mjs. DC Animation est le cinquieme univers dont la source
   ...
   traductions-witcher.mjs. Ne pas editer a la main : relancer le script.
   Les vingt-deux jeux gardent leur titre. */`;

let sortie = SRC;
sortie = remplaceBloc(sortie, 'window.CG=', CG_FR);
sortie = remplaceBloc(sortie, 'window.WITCHER=', W_FR);
...
const PAGE = lire('_proto/en-witcher.html');
...
let pageFR = pageTraduite
  ...
  .replace(/(<script\s+src=")data-witcher-en\.js(")/, '$1data-witcher.js$2')
   ...
   tombent sous le pixel. Même nombre de lignes que le bloc anglais. */
{
  ...
  if (!re.test(pageFR)) throw new Error('pas de sélecteur de langue dans en-witcher.html');
  pageFR = pageFR.replace(re, (bloc) => {
    const lignes = bloc.split('\n');
    const out = [
      '<a class="icon-btn lang-btn" href="en-witcher.html" aria-label="Switch to English" title="Switch to English">',
...
console.log(`  retrouvés TMDB : ${W_RETROUVES.length} titres et sous-items, `
  + `gabarits ${gabaritsVus.size}/${W_GABARITS.length}`);
...
fs.writeFileSync(path.join(RACINE, '_proto/a-traduire-witcher.json'),
...
if (!CHECK) {
  ...
  fs.writeFileSync(path.join(RACINE, '_proto/data-witcher.js'), sortie, 'utf8');
  fs.writeFileSync(path.join(RACINE, '_proto/e-witcher.html'), pageFR, 'utf8');
  console.log('\n  → _proto/data-witcher.js\n  → _proto/e-witcher.html');
  fs.writeFileSync(path.join(RACINE, '_proto/a-relire-witcher.json'),
