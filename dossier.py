...
@media(max-width:640px){{
  .it,.screen{{grid-template-columns:22px 1fr;gap:.15rem .65rem;align-items:start;padding:.6rem .7rem}}
  .ck,.ckx{{grid-column:1;grid-row:1;margin-top:.1rem}}
  .tt{{grid-column:2;grid-row:1;font-size:.88rem}}
  .dt{{grid-column:2;grid-row:2}}
  .kd{{grid-column:2;grid-row:3;justify-self:start;white-space:normal}}
  .note{{grid-column:2;grid-row:4}}
  .screen .tt{{grid-row:1}}
  .prog{{gap:.5rem}}
  .pnum{{order:1;flex:1 0 100%}}
  .pbar{{order:2}}
}}
</style>
</head>
